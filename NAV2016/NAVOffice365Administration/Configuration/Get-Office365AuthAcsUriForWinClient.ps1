<#
  SYNOPSIS
    This cmdlet enables the possibility for the Microsoft Dynamics NAV Windows Client to be configured
    for Office 365 user authentication.

  DESCRIPTION
    The cmdlet configures the Windows Azure Active Directory tenant by adding a new application for the 
    Microsoft Dynamics NAV Windows Client. Once this is performed, it returns the sign in page link that 
    needs to be configured in the ACSUri setting in the configuration file.

  .PARAMETER Office365AdministratorEmail
    Specifies the Office 365 administrator email address that is needed for the Windows Azure Active Directory configuration. 
  .PARAMETER Office365AdministratorPassword
    Specifies the Office 365 administrator password, in a SecureString form.
#>
function Get-Office365AuthAcsUriForWinClient
{
    [CmdletBinding()]
    PARAM(
        [Parameter(Mandatory=$false)]
        [ValidateNotNullOrEmpty()]
        [string] $Office365AdministratorEmail,

        [parameter(Mandatory=$false)]
        [ValidateNotNull()]
        [SecureString] $Office365AdministratorPassword
    )
    PROCESS
    {
        # Check if the Office365 connection is already established by looking up the . If not, connect.
        $companyInformation = Get-MsolCompanyInformation -ErrorVariable connectionError -ErrorAction SilentlyContinue
        if ($connectionError)
        {
            Connect-Office365 -Office365UserName $Office365AdministratorEmail -Office365Password $Office365AdministratorPassword -ErrorAction Stop
            $companyInformation = Get-MsolCompanyInformation
        }
        
        # Get the user's AAD tenant domain
        $aadTenantDomain = $companyInformation.InitialDomain

        # 1. Create or update the service principal for the Win Client.
        $windowsClientIdUri = "http://dynamicsnavwinclient/"
        Set-AadServicePrincipalForSingleSignOn -AppIdUri $windowsClientIdUri -AppReplyAddresses $(, $windowsClientIdUri) -ServicePrincipalDisplayName "Microsoft.Dynamics.Nav.WindowsClient" -ErrorAction Stop

        # 2. create and return the ACS Uri used by the windows client
        return "https://login.windows.net/$aadTenantDomain/wsfed?wa=wsignin1.0%26wtrealm=$windowsClientIdUri%26wreply=$windowsClientIdUri"
    }
}
# SIG # Begin signature block
# MIIa6AYJKoZIhvcNAQcCoIIa2TCCGtUCAQExCzAJBgUrDgMCGgUAMGkGCisGAQQB
# gjcCAQSgWzBZMDQGCisGAQQBgjcCAR4wJgIDAQAABBAfzDtgWUsITrck0sYpfvNR
# AgEAAgEAAgEAAgEAAgEAMCEwCQYFKw4DAhoFAAQUruaXOLk2RjIZwlzXo3vuRrG/
# kiigghWCMIIEwzCCA6ugAwIBAgITMwAAAIliDZ6V02FrqAAAAAAAiTANBgkqhkiG
...
# HAYKKwYBBAGCNwIBCzEOMAwGCisGAQQBgjcCARUwIwYJKoZIhvcNAQkEMRYEFP2P
# WelK4fKxd7QuuPj4d2BtsA9VMIGIBgorBgEEAYI3AgEMMXoweKBagFgATQBpAGMA
...
# bWljcm9zb2Z0LmNvbTANBgkqhkiG9w0BAQEFAASCAQB2BydIfFZHYaCM3cy0/Pqq
# TBjbM1WMq/v9dU8V5Mrdlj06hGlY53XtpsIrspzkv+kSsv8T6usJ4XwWV0tkd5DM
# 6YoseXmthRVgAVGPXC99czs2LRtYFmrWN+QUhFK4BEny8f2dzNEcj0cbybe0reRm
# UIVyHI109Mz+eYtHqx1flpRg/tEJ8aNNmWONo4j74T7r38dJbbnex3AMUkRzwHZD
# v2hlqwjpmXL+lsaHJ2lZGmm/JUKrYanZ2kNQ3PZpp+iUEf+JgU8I06aFOsi+psz9
# hVuCCYICKNbH3LENz/wC252T+2kD9ci4/r/N3n2y1ZAYDUTXYxiNWSIU9mGa9RtH
...
# NDI1WjAjBgkqhkiG9w0BCQQxFgQUYQBTXS7B0Te0mHMd7PO/uGs8+8gwDQYJKoZI
# hvcNAQEFBQAEggEAfKlt/SbhvI9F6THDsJtHyfkHxqT+fklHM0yBzjMW9CDxl/1e
# /UrkC7a4MLrAApgagEBsGpU8kwF4zKeM7pZ93Yrx93DhrywpbfGSlDbFIniZk/JQ
# khcDin4t7iAD8u2eS79koa760VkmGb0e/CD7KI74DyQzu2egGEWyWpTf9r8DEUqf
# HJyaC33j/XWARs8ihwiGE2Wg4xHAoQTlz/rvglLFycfwd4+OXZE2NVo9jzFBmzX5
# WP+07D2z++zghbMnhkKxQOfUEYL+V+SHurKcPcL9APP+1wodDkoekIis5Hn0cvbx
# lzdmW5Ld1K4e7hY3P8f7rL+afQCDNKWLDa+mFw==
