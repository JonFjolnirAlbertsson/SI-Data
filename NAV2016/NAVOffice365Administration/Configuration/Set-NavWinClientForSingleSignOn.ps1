<#
.SYNOPSIS
   This cmdlet updates the default client configuration with Single Sign On settings
...
.PARAMETER AcsUri
    The AscUri value.
#>
function Set-NavWinClientForSingleSignOn
{
    [CmdletBinding()]
    param(
    ...
    PROCESS
    {
        # Find latest installation in the program data folder.
        $installationPath = (Get-ChildItem -Path $([System.IO.Path]::Combine($env:ProgramData,'Microsoft\Microsoft Dynamics NAV')) | ?{ $_.PSIsContainer } | sort Name | select -Last 1)

        # Update ClientUserSettings.Config
        $configFilePath = [System.IO.Path]::Combine($installationPath.FullName,"ClientUserSettings.config")
        if (Test-Path -Path $configFilePath)
        {
            [xml]$configData = Get-Content -Path $configFilePath
            if (!$configData)
            {
                Write-Error "$configFilePath contails invalid data and cannot be updated."
            }
            else
            {
                $configData.SelectSingleNode("//add[@key='ClientServicesCredentialType']").SetAttribute("value","AccessControlService")
                $configData.SelectSingleNode("//add[@key='ACSUri']").SetAttribute("value",$AcsUri)
                $configData.Save($configFilePath)
                Write-Verbose "$configFilePath successfully updated."
            }
        }
        else
        {
            Write-Warning "ClientUserSettings.config was not found in $($installationPath.FullName). Configuration file was not updated."
        }
    }
}

Export-ModuleMember Set-NavWinClientForSingleSignOn
# SIG # Begin signature block
# MIIa6AYJKoZIhvcNAQcCoIIa2TCCGtUCAQExCzAJBgUrDgMCGgUAMGkGCisGAQQB
# gjcCAQSgWzBZMDQGCisGAQQBgjcCAR4wJgIDAQAABBAfzDtgWUsITrck0sYpfvNR
# AgEAAgEAAgEAAgEAAgEAMCEwCQYFKw4DAhoFAAQUVPEJTsTejbXNveBMFYogmXWP
# iZ+gghWCMIIEwzCCA6ugAwIBAgITMwAAAIliDZ6V02FrqAAAAAAAiTANBgkqhkiG
...
# +Kep5UhLGfYslFI6BnWz9UAuMIGIBgorBgEEAYI3AgEMMXoweKBagFgATQBpAGMA
...
# bWljcm9zb2Z0LmNvbTANBgkqhkiG9w0BAQEFAASCAQCPTXX3JFqrJ/9kuIpNroz9
# DO+IiJoteTqgKKHv2HLvkf3a+YIzwSNO3neqL5LTQuPE22GY0Kq3OETV3y+WMbRo
# CnnRoER9nMaznjngweJSNGdPUkx5NBDoQAIGqqJj+yjirwuhcRrY4HvIzLKTwUdy
# xKt6RsxygV3Zy7fkaEJQ4cf36Zg2pMA+ilvW2uweQ8opMzIawZODh/Vssz2VSYI5
# TD1+XCBWBhuANDxApEQlxOkVZ6tAlcZFMCF9gSHdY5sX6uaU5LwudzW1d/9rZRYt
# R2gKkCG9BpVFv7zjP28Sjyi4AUxzGrUtp8eenMTJ9hB8QFWzmxSDdu5LMb1aFUNh
...
# NDI1WjAjBgkqhkiG9w0BCQQxFgQUmauOvqhBX+ykw2/vJOLdJpiGyl8wDQYJKoZI
# hvcNAQEFBQAEggEALLbS7t5GNfw4oUaVlSDQ+dGoHSB0tJ1i8IG9PP93bCeNHgiH
# vdXdjitFv87glD2jwkoZcqoN6gEK37IxiuJhSRGuRG8XTHi398Nu+QRP3tAEOJja
# +IjGtIENLBv9gF0SF04bwXZ/+JpaiD8A1dAl/tl3GJkvR+9nZ6vFzeW7tZ0IVcfB
# XiJnxn2QhwpYAiSBaNst6CiE3/MTRgBTwTktkSXB1Tj4KQ7IRcbg3hSHeGLQAcTX
# BlDwP+SY0Q6bHpcCgZzWq7kmqYhmnIl0DyjR20zeGbZH61oktmDeWwyTBzql/IcT
# 8AehVUAwL4277q91rcKYwTnItlNxVwpz75kPvQ==
