<#
 .SYNOPSIS
   This cmdlet restores the CustomSettings.config file for NavServerInstance and
   the web.config files for NavWebServerInstance
...
#>
function Restore-NavConfiguration
{
    [CmdletBinding()]
    param(
    ...
    {
		# Get Backup log
		$logFileName = "ConfigBackupLog.txt"
		$logFilePath = Join-Path $BackupDirectory "ConfigBackupLog.txt"
		
		if (!(Test-Path -PathType Leaf $logFilePath))
		{
			Write-Error "Could not find log file for backup (The file '$logFilePath' does not exist)."
			return
		}

		$logFile = Get-Content $logFilePath

        # restore config file for each line entry
		foreach ($line in $logfile)
		{
            # Format is FROM-><orig file path> TO-><orig destination file>
            $Length = $line.Length            
			$ToStart = $line.IndexOf("TO->")
            # There should be 4 letters for "TO->" after tostart and then at least 1 character for to-lists
            if (($ToStart -gt 7) -and ($ToStart+4+1 -gt $Length))
            {
                Write-Error "Corrupt log file"
            }

			$FromFilePath = $Line.SubString(6,$ToStart-6)
            if (!(Test-Path  $FromFilePath))
            {
                Write-Error "Expected the config file $FromFilePath to be already there."
                return
            }

			$ToFilePath = $Line.SubString($ToStart+4, $Length-$ToStart-4)
            if (!(Test-Path  $ToFilePath))
            {
                Write-Error "Could not find backed up file $ToFilePath"
                return
            }

            Copy-Item -Path $ToFilePath -Destination  $FromFilePath -force
		}
	}
}
Export-ModuleMember Restore-NavConfiguration


# SIG # Begin signature block
# MIIa6AYJKoZIhvcNAQcCoIIa2TCCGtUCAQExCzAJBgUrDgMCGgUAMGkGCisGAQQB
# gjcCAQSgWzBZMDQGCisGAQQBgjcCAR4wJgIDAQAABBAfzDtgWUsITrck0sYpfvNR
# AgEAAgEAAgEAAgEAAgEAMCEwCQYFKw4DAhoFAAQUE5kwCi3PE4EDQ4YG+NfiJfys
# vPagghWCMIIEwzCCA6ugAwIBAgITMwAAAItMBnEMICzfOwAAAAAAizANBgkqhkiG
...
# HAYKKwYBBAGCNwIBCzEOMAwGCisGAQQBgjcCARUwIwYJKoZIhvcNAQkEMRYEFBvq
# auEW0hO1BhWtClR8LNsvkV4dMIGIBgorBgEEAYI3AgEMMXoweKBagFgATQBpAGMA
...
# bWljcm9zb2Z0LmNvbTANBgkqhkiG9w0BAQEFAASCAQCCUhjYyQQyeJJ1bcmLiWnK
# QNjzvrZ5iN5NGH+67avLJNTbMF8gNSRAS+XkKjqsBUbiGW9NEey5VgBerAZItKCa
# GTrWsPWYdgp8x6PQwgRLNSNENMs+CltovLoPHl0GrceYnV5+nIPAOwuqVTV32P3E
# ZciGvCjefeJ8OK4EXYHEa1bAeYVhZMDPiIsISlOiOyE0tKEWZB6ccgH1uerLjD3q
# C19M2Usum3DoE0ITgMw2v4nsoBlgSkoFkgCBjWXEy8NNp/YVv6DvPvXjAln4+ple
# YhTO87xzh2wGMK7kZI/A7IujrES+FvItvDQFzi6FOlPyIqKwgNhFHE/hMN0PUgbI
...
# NDI1WjAjBgkqhkiG9w0BCQQxFgQUfJtVv9Sj4gBLwKOxSrQE2n4kngcwDQYJKoZI
# hvcNAQEFBQAEggEAe5mhnfteQWhq3ktdFfwjopwGINnVMeoe42zZ8tpSk5HxJyn/
# j+vDqWvsEVZMptbUkvrD25wXZyYfYJOx/s89bSeUVQFa8XwKT74a5OBgqyzgtl09
# it5kVgAxkJR3RUrTE6STqTNb1xdOBsdEdVA2p+Jb5nsEFWy1pnnMSjHt5cO4GisM
# sw6+O9n9d2B8FzPzAApVF2529cxHLcxThMei84ThB9AJJDn3dTWwGIDIo6O5guD7
# eNQBAlyoR+pbyHBSysLkIX0H8h4d4qHNa0VtE8Gc1vul05eiTL09EllbRD9o1Fuy
# mRpSIxasHzbC45XAraf49qjWOMucXdAdImdz2g==
